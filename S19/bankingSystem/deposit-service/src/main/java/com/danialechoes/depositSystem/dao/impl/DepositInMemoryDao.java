...
import com.danialechoes.depositSystem.exception.DepositNotFoundException;
...
public class DepositInMemoryDao implements DepositDao {
    ...
    private Deposit deepCopy(Deposit deposit) {
        ...
        copy.setCustomerId(deposit.getCustomerId()); // Assuming Customer is immutable or has its own deep copy logic
    ...
    public List<Deposit> findByCustomerId(Long id) {
        return deposits.values().stream()
                .filter(deposit -> deposit.getCustomerId() != null && deposit.getCustomerId().equals(id))
                .map(this::deepCopy)
                .toList();
    }
